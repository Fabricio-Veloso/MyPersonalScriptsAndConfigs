...
class Checker:
    ...
    def check(self, profile_name: str) -> PlanResult:
        platform_name = detect_platform()
        profile = self.profile_loader.load(profile_name)
        steps: list[PlanStep] = []

        for module_name in profile.modules:
            result = self.module_registry[module_name].check()
            steps.append(PlanStep(module_name, result.status_label, result.details or result.status_label))
